...
#define ENET_MTU_MAX				(ETH_DATA_LEN + BRCM_MAX_TAG_LEN) /* DSA tag overhead */
...
static void bcm4908_enet_dma_reset(struct bcm4908_enet *enet)
{
	 ...
	 * channels may be active. Blindly writing 0 to CFG won't stop
	 * an in-flight DMA burst. Set PKT_HALT + BURST_HALT first.
	 */
	for (i = 0; i < ARRAY_SIZE(rings); i++)
		enet_write(enet, rings[i]->cfg_block + ENET_DMA_CH_CFG,
			   ENET_DMA_CH_CFG_PKT_HALT | ENET_DMA_CH_CFG_BURST_HALT);

	usleep_range(100, 200);

	/* Poll for channels to stop (ENABLE bit clears when halted) */
	for (i = 0; i < ARRAY_SIZE(rings); i++) {
		deadline = jiffies + usecs_to_jiffies(10000);
		do {
			cfg = enet_read(enet, rings[i]->cfg_block + ENET_DMA_CH_CFG);
			if (!(cfg & ENET_DMA_CH_CFG_ENABLE))
				break;
			usleep_range(50, 100);
		} while (!time_after_eq(jiffies, deadline));

		if (cfg & ENET_DMA_CH_CFG_ENABLE)
			dev_warn(enet->dev, "DMA CH%d still enabled after halt (CFG=0x%08x)\n",
				 i, cfg);
	}

	/* Now disable channels and master controller */
	for (i = 0; i < ARRAY_SIZE(rings); i++)
	...
	/* Assert channel reset with delay for hardware to settle */
	enet_write(enet, ENET_DMA_CTRL_CHANNEL_RESET, 0x3);
	usleep_range(100, 200);
	enet_write(enet, ENET_DMA_CTRL_CHANNEL_RESET, 0);
	usleep_range(50, 100);

	/* Clear channel state RAM */
	for (i = 0; i < ARRAY_SIZE(rings); i++) {
	...
	/* Verify state RAM cleared */
	for (i = 0; i < ARRAY_SIZE(rings); i++) {
		u32 v = enet_read(enet, rings[i]->st_ram_block + ENET_DMA_CH_STATE_RAM_STATE_DATA);

		if (v)
			dev_warn(enet->dev, "DMA CH%d state RAM not zero after reset: 0x%08x\n",
				 i, v);
	}
...
static int bcm4908_enet_open(struct net_device *netdev)
{
	...
	/* Reset DMA and GMAC BEFORE registering IRQs.
	 * On devices where U-Boot initialized the RDP engine (e.g. AP630),
	 * the ENET interrupt lines may be asserted. If we request_irq first,
