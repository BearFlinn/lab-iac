...
static void f_configure_runner(S_DPI_CFG  *pCfg, rdp_bpm_cfg_params *p_bpm_cfg_params)
{
    ...
    rdd_init_params.ddr_bm_ptr       = (uint8_t *) pCfg->runner_tm_base_addr;
    rdd_init_params.ddr_bm_phys  = (uint32_t) pCfg->runner_tm_base_addr_phys;
    rdd_init_params.ddr_fm_ptr = (uint8_t *) pCfg->runner_mc_base_addr;
    rdd_init_params.ddr_fm_phys = (uint32_t) pCfg->runner_mc_base_addr_phys;
    /* ddr_runner_table only needs to be 512KB aligned, but we make 1 MB anyway */
    rdd_init_params.ddr_fm_ptr = (uint8_t *) ROUND_UP_MB(pCfg->runner_tm_base_addr +
        (p_bpm_cfg_params->bpm_def_thresh * rdp_bpm_buf_size()));
    rdd_init_params.ddr_fm_phys = RDD_RSV_VIRT_TO_PHYS(pCfg->runner_tm_base_addr, 
        pCfg->runner_tm_base_addr_phys, rdd_init_params.ddr_fm_ptr);
