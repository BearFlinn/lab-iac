...
What's left is a small closeout list (see **Remaining Work** at the top of Phase-level sections): purchase an off-the-shelf router ([ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md)) and configure VLANs once it arrives, join the Tower PC to the cluster as a plain worker, build out the new GPU inference host, mount remaining APs, and replace UPS batteries.
...
- **Data safety first** — ~~back up the 3TB drive before touching any storage~~ (resolved — [ADR-007](../../docs/decisions/007-3tb-data-drive-direct-to-pool.md))
...
**Staging approach (executed):** Critical workloads ran on a staging VM on the R730 during the cluster rebuild. Decommissioned 2026-04-09 after Phase 7b moved landing-page, caz-portfolio, and resume-site onto the new K8s cluster ([ADR-002 — Superseded](../../docs/decisions/002-r730-staging-vm-for-migration.md)).
...
- [x] ~~Remove 3TB drive from mini PC~~ — **physically removed 2026-03-26, data preserved via [ADR-007](../../docs/decisions/007-3tb-data-drive-direct-to-pool.md)**
...
- [x] ~~Audit Tower PC PSU~~ — **determined insufficient for the planned 3-GPU inference fleet. Tower PC repurposed as plain K8s worker; GPU inference moves to a separate new-build host. See [ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md).**
...
- [x] ~~Back up critical data before migration:~~
  - ~~3TB drive (removed from mini PC 2026-03-26)~~ — **mounted directly into R730xd MergerFS pool (bay 8). Data preserved in-place, no separate backup needed. See [ADR-007](../../docs/decisions/007-3tb-data-drive-direct-to-pool.md).**
  - [x] ~~Palworld server data (Optiplex / old deb-web)~~ — **save data + config backed up 2026-04-03 to `~/Backups/deb-web/` (52MB). Service decommissioned indefinitely, see [ADR-022](../../docs/decisions/022-palworld-decommissioned.md).**
...
- [x] ~~DNS/networking dependencies~~ — **flattened onto current topology: NFS export scoped to lab subnet via the `r730xd-nfs-server` role; VPS → cluster via dedicated WG tunnel ([ADR-019](../../docs/decisions/019-ingress-and-tls-termination.md)), not NetBird subnet routes.**
...
- Non-lab network (bedroom, garage) staying on existing switch chain for now. See [ADR-008](../../docs/decisions/008-keep-existing-switch-chain-for-home.md).
- [x] ~~Closet ↔ AP230 location~~ — **cable run in place; AP230 mounting still pending (see Phase 4B).**
- [x] ~~Closet ↔ Xfinity gateway (living room)~~ — **uplink in place; Xfinity gateway is still doing the routing (flat L2 in the lab) until the off-the-shelf router from [ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md) arrives.**

### 1C: Set Up Switch & Network

The SR2024 is the lab backbone. VLANs are **deferred until the off-the-shelf router arrives** ([ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md)) — configuring VLANs on top of the Xfinity gateway's routing buys us very little, and the purchased router will handle inter-VLAN routing much more cleanly than any DIY option.
...
- [x] ~~Drain Tower PC from old K8s~~ — **done 2026-04-03. Force-deleted from the old cluster. Will rejoin the new cluster as a plain worker ([ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md)).**
...
~~Skipped — APC RS 1500 batteries are dead and need replacement. Proceeding without UPS for now. See [ADR-006](../../docs/decisions/006-proceed-without-ups.md).~~
...
- [x] ~~Set up ZFS pool on 3×2TB drives (migrated from tower-pc) for latency-sensitive workloads~~ — **done 2026-04-03. ZFS raidz1 pool `tank` (3×2TB, bays 9+10+11) mounted at `/mnt/zfs`. ~3.6TB usable. lz4 compression, 4GB ARC max. Monitoring: `check-zfs.sh` (tier 1, every 5 min), Prometheus alert rules (degraded/capacity/errors/scrub). See [ADR-004](../../docs/decisions/004-zfs-iscsi-for-k8s-storage.md). Deployed via `ansible/playbooks/r730xd-zfs.yml`.**
- [x] ~~Set up iSCSI off ZFS for K8s cluster storage~~ — **done. `democratic-csi` provides `iscsi-zfs` (default StorageClass) + `nfs-mergerfs`. See [ADR-015](../../docs/decisions/015-dynamic-storage-provisioning.md) and live cluster (`kubectl get sc`).**
...
- [x] ~~Set up PXE boot server~~ — **superseded by [ADR-013](../../docs/decisions/013-local-disk-over-pxe-boot.md): K8s nodes boot from local disk, no PXE infrastructure. Simpler, faster, fewer moving parts. ADR-005 (NFS-root for PXE) is also superseded.**
...
- ~~Install NetBird for VPN access~~ — **N/A: R730xd is internal-only; it terminates the dedicated WG ingress tunnel instead ([ADR-019](../../docs/decisions/019-ingress-and-tls-termination.md)).**
- [x] ~~Verify NFS is accessible from K8s nodes~~ — **done. `nfs-mergerfs` StorageClass live via `democratic-csi`.**
- [x] ~~**Stand up staging VM**~~ ([ADR-002 — Superseded](../../docs/decisions/002-r730-staging-vm-for-migration.md)) — **done 2026-03-28, decommissioned 2026-04-09. Served landing-page, caz-portfolio, resume-site during the cluster rebuild; all three moved back onto the new K8s cluster via Flux in Phase 7b (commits `7cb5af7`, `82555ec`, `3c984f7`). Staging VM teardown: commit `15d5f14`; playbooks preserved in `archive/staging-vm/`.**
...
See [ADR-004](../../docs/decisions/004-zfs-iscsi-for-k8s-storage.md) (architecture) and [ADR-015](../../docs/decisions/015-dynamic-storage-provisioning.md) (dynamic provisioning via democratic-csi).
...
- [x] ~~**Palworld server**~~ — **decommissioned indefinitely ([ADR-022](../../docs/decisions/022-palworld-decommissioned.md)). Save data backed up; VPS UDP forward will be removed alongside this doc update.**
- [x] ~~**GitHub Actions runner**~~ — **old self-hosted runner retired; replaced by in-cluster ARC v2 ([ADR-017](../../docs/decisions/017-arc-v2-github-runners.md)) with a custom runner image built via Argo Workflows + Kaniko (Phase 8).**
...
- [x] ~~Remove tower-pc from K8s~~ — **done 2026-04-03. Will rejoin the new cluster as a plain worker per [ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md).**
...
- [x] ~~Update VPS proxy config~~ — **VPS Caddy now routes `*.bearflinn.com` through the WG ingress tunnel to the K8s NodePort per [ADR-019](../../docs/decisions/019-ingress-and-tls-termination.md). Staging VM route retired.**
...
**Revised** by [ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md): Tower PC will join as a plain K8s worker. No router role, no GPU role. The GPU inference fleet moves to a separate new-build host (tracked as "pending" in `../../docs/hardware.md`). Routing moves to an off-the-shelf router when purchased.
...
~~Originally planned as Debian arm64 router ([ADR-003](../../docs/decisions/003-ap630-as-router.md)). Closed out — hardware bandwidth ceiling of 95 Mbps is not viable for GbE WAN routing ([ADR-010](../../docs/decisions/010-ap630-iudma-limit-requires-rdp.md), [ADR-011](../../docs/decisions/011-ap630-restored-to-stock-wifi-ap.md)).~~
...
- [x] ~~AP630 restored to stock HiveOS~~ — **IQ Engine 10.6r7 (2026-04-03). See [ADR-011](../../docs/decisions/011-ap630-restored-to-stock-wifi-ap.md).**
- [ ] Mount AP230 in central location (starting with AP230 only — [ADR-009](../../docs/decisions/009-start-with-ap230-only.md))
...
- [x] ~~Update VPS proxy inventory~~ — **Palworld UDP forward and `netbird_palworld_ip` removed (2026-04-17) alongside [ADR-022](../../docs/decisions/022-palworld-decommissioned.md).**
...
|------|--------|------------|
| ~~3TB drive data lost~~ | ~~High~~ | ~~Resolved: drive mounted directly into MergerFS pool ([ADR-007](../../docs/decisions/007-3tb-data-drive-direct-to-pool.md)). SnapRAID parity protects it.~~ |
...
| ~~Tower PSU can't handle 3 GPUs~~ | ~~Medium~~ | ~~Confirmed insufficient. GPU fleet moves to a separate new-build host; Tower PC becomes plain K8s worker ([ADR-021](../../docs/decisions/021-off-the-shelf-router-tower-pc-as-worker.md)).~~ |
