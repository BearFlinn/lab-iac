...
- **Tower PC is the router again.** This re-couples routing availability with GPU inference workload, but the trade-offs documented in ADR-001 remain acceptable for a homelab.
...
- **Significant engineering investment is closed out.** The Debian porting work (custom kernel, DTS, PMB driver patches, RDP reverse engineering) is preserved in `ap630-debian/` for reference but is no longer actively developed.
- **NAND backups remain in `~/Backups/`** and the restore script at `ap630-debian/scripts/restore-stock.expect` is tested and working, should it ever be needed again.
