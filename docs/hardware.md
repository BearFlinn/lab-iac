...
| Role | Storage backbone, observability stack, foundation stores (Postgres / Redis / MinIO Obs / MinIO Bulk / OpenBao), ingress tunnel terminator |
...
| ZFS pool `tank` (`/mnt/zfs`) | 3×2TB raidz1 (bays 9/10/11) | ~3.6 TB | Latency-sensitive services (Postgres/Redis/MinIO Obs/OpenBao/Prometheus/Loki/Tempo/Grafana), K8s `iscsi-zfs` StorageClass (via democratic-csi) |
